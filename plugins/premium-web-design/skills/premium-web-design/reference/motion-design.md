...
## Motion Tokens (copy-paste starter)

Define once in `:root`, use everywhere — this is what makes motion feel like one system instead of scattered decisions:

```css
:root {
  /* Durations */
  --duration-instant: 100ms;   /* button press, toggle, color change */
  --duration-fast: 200ms;      /* hover states, tooltips, menus */
  --duration-normal: 350ms;    /* accordions, modals, drawers */
  --duration-slow: 600ms;      /* page load, hero reveals */

  /* Easing */
  --ease-out-quart: cubic-bezier(0.25, 1, 0.5, 1);   /* default for entrances */
  --ease-out-expo: cubic-bezier(0.16, 1, 0.3, 1);    /* snappy, confident */
  --ease-in-quart: cubic-bezier(0.7, 0, 0.84, 0);    /* exits (use ~75% of enter duration) */
  --ease-in-out: cubic-bezier(0.65, 0, 0.35, 1);     /* state toggles */

  /* Composite transitions */
  --transition-hover: color var(--duration-fast) var(--ease-out-quart),
                      background-color var(--duration-fast) var(--ease-out-quart),
                      transform var(--duration-fast) var(--ease-out-quart);
  --transition-reveal: opacity var(--duration-slow) var(--ease-out-expo),
                       transform var(--duration-slow) var(--ease-out-expo);

  /* Stagger */
  --stagger-step: 50ms;  /* animation-delay: calc(var(--i, 0) * var(--stagger-step)) */
}
```
