...
|------|---------|---------|
| `brand.json` | Colors, fonts, assets | PPTX Generator, diagram (excalidraw canvas), **dataviz** (charts: pass `brand.json` to `dataviz/engine/viz.py --palette`; it maps the colors to chart roles and validates them for WCAG contrast + colourblind - the accessibility check this plugin does not do) |
